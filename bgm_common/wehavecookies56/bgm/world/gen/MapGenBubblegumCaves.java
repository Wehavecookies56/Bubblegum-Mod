package wehavecookies56.bgm.world.gen;
...
public class MapGenBubblegumCaves extends MapGenBase
{
    /**
    ...
    {
        BiomeGenBase biome = worldObj.getBiomeGenForCoords(x + chunkX * 16, z + chunkZ * 16);
        int top    = (isExceptionBiome(biome) ? AddedBlocks.PinkGum1.blockID : biome.topBlock);
        int filler = (isExceptionBiome(biome) ? AddedBlocks.PinkGum1.blockID  : biome.fillerBlock);
        int block  = data[index];

        if (block == AddedBlocks.BlueGum1.blockID || block == filler || block == top)
        {
            if (y < 10)
            {
                data[index] = (byte)AddedBlocks.BlueGum2.blockID;
            }
            else
            {
                data[index] = 0;
